/**
 * KSP / KOMENDA STOŁECZNA POLICJI — SYSTEM RP
...
const DB_FILE = path.join(__dirname, process.env.DB_FILE || 'ksp_mdt_data.json');
...
const COLLECTIONS = {
  ...
  bolos: { prefix: 'BOLO', label: 'Poszukiwani / BOLO' },
...
app.get('/auth/callback', async (req, res) => {
  try {
    ...
    const hasAccess = roles.includes(DISCORD_ROLE_ID);
    const isAdmin = DISCORD_ADMIN_ROLE_ID ? roles.includes(DISCORD_ADMIN_ROLE_ID) : true;
...
app.get('/api/dashboard', requireAuth, (req, res) => {
  const activeBolos = db.bolos.filter(item => item.status !== 'Zamknięte');
  ...
  res.json({
    ...
    activeBolos: activeBolos.slice(0, 8),
...
<head>
  ...
  <title>KSP — Komenda Stołeczna Policji | System Informacyjny</title>
  <style>
    :root {
      --bg-0: #050a12;
      --bg-1: #08111e;
...
<body>
  <div id="loginScreen" class="login-screen">
    <div class="login-card">
      ...
      <h1>KSP • POLICJA</h1>
      <p class="subtitle">
        Nieoficjalny system informacyjny RP inspirowany Komendą Stołeczną Policji.
        Dostęp wymaga konta Discord oraz właściwej roli na wskazanym serwerze.
      </p>
      ...
      <div class="login-features">
        <div class="login-feature">Weryfikacja serwera i roli</div>
        <div class="login-feature">Szyfrowana sesja HTTP-only</div>
        <div class="login-feature">Pełny dziennik operacji</div>
      </div>
    </div>
  </div>
  ...
  <div id="app" class="app hidden">
    <aside id="sidebar" class="sidebar">
      ...
      <div style="margin:0 4px;padding:9px 11px;border-radius:10px;background:rgba(47,140,255,.07);border:1px solid rgba(103,178,255,.16);font-size:10px;font-weight:850;letter-spacing:1.4px;color:#9bcfff;text-align:center">POLICJA • SYSTEM WEWNĘTRZNY RP</div>

      <nav class="nav" id="navigation">
        <div class="nav-title">Centrum</div>
        <button class="nav-button active" data-page="dashboard">
          <span class="nav-icon">⌂</span> Panel główny
        </button>
        <button class="nav-button" data-page="dispatch">
          <span class="nav-icon">☎</span> Zgłoszenia / dyspozytornia
        </button>

        <div class="nav-title">Ewidencja</div>
        ...
        <div class="nav-title">Operacje</div>
        <button class="nav-button" data-page="reports">
          <span class="nav-icon">▤</span> Raporty
        </button>
        <button class="nav-button" data-page="bolos">
          <span class="nav-icon">⚠</span> Poszukiwani / BOLO
        </button>
    ...
    <main class="main">
      <header class="topbar">
        ...
        <div class="global-search-wrap">
          ...
          <input id="globalSearch" class="global-search" placeholder="Szukaj osoby, pojazdu, sprawy, numeru rejestracyjnego...">
  ...
  <script>
    ...
    var pageConfig = {
      citizens: {
        ...
        fields: [
          ...
          { key: 'gender', label: 'Płeć', type: 'select', options: ['Nie podano', 'Kobieta', 'Mężczyzna', 'Inna'] },
          { key: 'documentNo', label: 'Numer dokumentu' },
          { key: 'phone', label: 'Telefon' },
          { key: 'address', label: 'Adres', full: true },
          { key: 'licenseNo', label: 'Numer prawa jazdy' },
          { key: 'licenseStatus', label: 'Status prawa jazdy', type: 'select', options: ['Brak', 'Ważne', 'Zawieszone', 'Cofnięte'] },
          { key: 'status', label: 'Status osoby', type: 'select', options: ['Brak uwag', 'Obserwowany', 'Poszukiwany', 'Zatrzymany'] },
      ...
      vehicles: {
        ...
        fields: [
          ...
          { key: 'insurance', label: 'Ubezpieczenie', type: 'select', options: ['Nieznane', 'Ważne', 'Nieważne', 'Brak'] },
          { key: 'inspection', label: 'Badanie techniczne', type: 'select', options: ['Nieznane', 'Ważne', 'Nieważne', 'Brak'] },
    ...
    function statusBadge(value) {
      ...
      else if (low.includes('średni') || low.includes('przyję')) color = 'purple';
    ...
    function setUser(user) {
      ...
      document.getElementById('sidebarUserRole').textContent =
        user.isAdmin ? 'Funkcjonariusz · pełny dostęp' : 'Funkcjonariusz';
    ...
    async function renderDashboard() {
      ...
      var cards = [
        ...
        ['Aktywne BOLO', stats.activeBolos, '⚠', '#ff5364', 'Wymagają natychmiastowej uwagi'],
          ...
          '<div style="display:flex;flex-direction:column;gap:16px">' +
            '<div class="panel">' +
              ...
              '<div class="panel-body"><div class="quick-grid">' +
                ...
                quickAction('⚠', 'Nadaj BOLO', 'Komunikat operacyjny', 'bolos') +
                quickAction('◇', 'Zabezpiecz dowód', 'Łańcuch przechowywania', 'evidence') +
                quickAction('☎', 'Przyjmij zgłoszenie', 'Nowe zdarzenie', 'dispatch') +
              '</div></div>' +
            '</div>' +
            dashboardListPanel('Aktywne komunikaty BOLO', 'Najwyższy priorytet', data.activeBolos, 'bolos') +
    ...
    function renderSettings() {
      var html = pageHeader(
        'Ustawienia systemu',
        'Informacje o sesji, bezpieczeństwie oraz kopiach zapasowych.',
        ''
      );
      ...
      html +=
        '<div class="settings-grid">' +
          ...
          '<div class="settings-card">' +
            ...
            '<p>System jest projektem roleplay inspirowanym polską Policją i nie jest oficjalnym serwisem KSP. Nie umieszczaj w nim prawdziwych danych osobowych.</p>' +
          '</div>' +
        '</div>';
    ...
    function buildInput(field, record) {
      ...
      if (field.type === 'citizen') {
        var citizens = '<option value="">— brak / nieznany —</option>';
        state.citizens.forEach(function (person) {
          citizens += '<option value="' + escapeHtml(person.id) + '"' +
            (String(value) === String(person.id) ? ' selected' : '') + '>' +
            escapeHtml((person.fullName || person.id) + ' · ' + (person.documentNo || person.id)) +
            '</option>';
        });
        return '<select class="form-control" name="' + field.key + '"' + required + '>' + citizens + '</select>';
      }

      if (field.type === 'vehicle') {
        var vehicles = '<option value="">— brak / nieznany —</option>';
        state.vehicles.forEach(function (vehicle) {
          vehicles += '<option value="' + escapeHtml(vehicle.id) + '"' +
            (String(value) === String(vehicle.id) ? ' selected' : '') + '>' +
            escapeHtml((vehicle.plate || vehicle.id) + ' · ' + (vehicle.make || '') + ' ' + (vehicle.model || '')) +
            '</option>';
        });
        return '<select class="form-control" name="' + field.key + '"' + required + '>' + vehicles + '</select>';
      }
    ...
    async function saveRecord(collection, id) {
      ...
      try {
        ...
        closeModal();
        await loadLookups();
        await navigate(state.currentPage === 'dashboard' ? 'dashboard' : collection);
    ...
    async function printRecord(collection, id) {
      try {
        ...
        popup.document.write(
          ...
          '</style></head><body><div class="head"><h1>KSP — Komenda Stołeczna Policji · ' + escapeHtml(title) + '</h1>' +
...
app.get('/health', (req, res) => {
  res.json({ ok: true, service: 'KSP System Informacyjny', time: new Date().toISOString() });
});
