...
app.post('/api/duty/start', requireAuth, (req, res) => {
  ...
  addAudit(req, 'DUTY_START', 'duty', record.userId, 'Rozpoczęto aktywną służbę');
  saveDatabase(db);
...
app.post('/api/duty/stop', requireAuth, (req, res) => {
  ...
  addAudit(req, 'DUTY_STOP', 'duty', record.userId, `Zakończono służbę (${Math.round(durationMs / 60000)} min)`);
  saveDatabase(db);
...
app.get('/api/dashboard', requireAuth, (req, res) => {
  ...
  res.json({
    stats: {
      ...
      activeDuty: db.duty.filter(item => Boolean(item.activeSince)).length
    },
  ...
  <script>
    ...
    function renderDutyView(data) {
      ...
      var action = current.active
        ...
        : '<button class="button success duty-main-button" onclick="startDuty()">▶ Rozpocznij służbę</button>';

      var html = pageHeader(
        'Aktywna służba',
        'Rozpoczynaj i kończ służbę. Każda zmiana dolicza się do łącznego czasu funkcjonariusza.',
        ...
        '<div class="duty-grid">' +
          '<div class="duty-summary ' + (current.active ? 'active' : '') + '">' +
            ...
            '<div class="duty-status-line"><span class="pulse-dot"></span>' +
              (current.active ? 'Służba aktywna od ' + escapeHtml(formatDate(current.activeSince, true)) : 'Poza służbą') +
            '</div>' +
          ...
          '<div class="panel duty-leaderboard">' +
            '<div class="panel-header"><div><h3>Ranking czasu służby</h3><span>Od największej liczby godzin do najmniejszej</span></div>' +
              '<span class="badge blue">' + leaderboard.length + ' osób</span></div>' +
            '<div class="table-wrap duty-table-wrap"><table><thead><tr>' +
              '<th>#</th><th>Funkcjonariusz</th><th>Status</th><th>Bieżąca zmiana</th><th>Łączny czas</th><th>Ostatnie zakończenie</th>' +
            '</tr></thead><tbody>' +
              (leaderboard.length ? leaderboard.map(function (item, index) {
                return '<tr>' +
                  ...
                  '<td><div class="duty-person">' + dutyAvatar(item) + '<div><strong>' + escapeHtml(item.displayName) +
                    '</strong><span>' + escapeHtml(item.userId) + '</span></div></div></td>' +
                  '<td>' + (item.active ? '<span class="badge green">Na służbie</span>' : '<span class="badge blue">Poza służbą</span>') + '</td>' +
                  ...
                  '<td>' + escapeHtml(item.lastEndedAt ? formatDate(item.lastEndedAt, true) : '—') + '</td>' +
                '</tr>';
    ...
    async function startDuty() {
      try {
        await api('/api/duty/start', { method: 'POST', body: '{}' });
        toast('Służba rozpoczęta', 'Czas jest teraz naliczany.', 'success');
        await renderDuty();
      } catch (error) {
